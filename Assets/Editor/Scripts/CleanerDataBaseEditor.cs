...
[CustomEditor(typeof(CleanersDataBase))]
public class CleanerDataBaseEditor : Editor
{
    ...
    {
        _dataBase = target as CleanersDataBase;

        _boosterDataFields = new List<string>();
        foreach (var field in typeof(CleanerData).GetFields(BindingFlags.NonPublic | BindingFlags.Instance))
        ...
        _dataBaseListInfo = allFields.ToList().Find((field) => field.FieldType.IsEquivalentTo(typeof(List<CleanerData>)));
    }
